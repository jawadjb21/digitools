...
const Navbar = ({ navLinksPromise, selectedProducts }) => {
    ...
    return (
        <div className='sticky top-0 z-50 bg-base-100 w-full'>
            <nav className='flex justify-between items-center mx-auto container my-3'>
                ...
                <h2 className='bg-linear-to-r from-[#4F39F6] to-[#627382] bg-clip-text inline-block text-transparent font-bold text-4xl md:text-5xl pb-1'>DigiTools</h2>
                <ul className='hidden md:flex justify-between items-center list-none gap-5'>
                    {navLinks.map(link => link.name !== 'Login' && <Link key={link.id} link={link}></Link>)}
                </ul>
                <div className='flex justify-between items-center'>
                    <div className="indicator">
                        {selectedProducts.length > 0 ? <span className="indicator-item badge badge-secondary mr-4">{selectedProducts.length}</span> : ""}
                        <button className='btn btn-ghost text-3xl'><IoIosCart /></button>
                    </div>
